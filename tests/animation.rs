...
//! The clock is pinned so each frame is exact rather than a race.
...
/// Whether any piece-coloured pixel of `side` appears inside one square.
...
fn play(from: Square, to: Square) -> (App, Instant) {
...
fn a_move_starts_a_slide_that_ends_on_its_own() {
    let (mut app, start) = play(Square::G1, Square::F3);
...
fn the_piece_passes_through_the_squares_between() {
    ...
    let (mut app, start) = play(Square::E2, Square::E4);
